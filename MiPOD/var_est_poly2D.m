function [ estimated_variance , estimated_expectation , residuals ] = var_est_poly2D(input_image,blk_size,degre,mode)
...
% Building the matrix H
% Here, the basis vectors are 2D monomial
[x1,y1] = meshgrid(linspace(0,1,blk_size_x) , linspace(0,1,blk_size_y));
H = zeros(blk_size_x*blk_size_y,q);
l=0;
for i = 0 : degre,
	for j =  0:degre-i, 
        if i < blk_size_x && j < blk_size_y
            l=l+1;
            H(:,l) = (x1(:)).^(i) .* (y1(:)).^(j);
        end,
	end
...
% Assuring conditionnement of regression matrix with orthonomalization
H(:,1) = H(:,1) ./norm(H(:,1));
for i=2:size(H,2);
	for j=1:i-1,
		H(:,i) = H(:,i) - H(:,j) * ( ( H(:,j)'*H(:,i) ) / ( H(:,j)'*H(:,j) ) );
	end,
	H(:,i) = H(:,i) ./norm(H(:,i));
end,
